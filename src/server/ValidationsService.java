...
public class ValidationsService {
    ...
    public static CustomerAbstract requestAndValidateCustomer(BufferedReader input, PrintWriter output) throws IOException {
        try {
            ...
            return CustomerFactory.createCustomer(custId, fullName, phoneNumber, customerType);
    ...
    public static String validateId(String s) throws IllegalArgumentException {
        if (s == null || s.trim().isEmpty()) throw new IllegalArgumentException("ID cannot be empty.");
        if (!s.matches("\\d+")) throw new IllegalArgumentException("ID must contain only digits.");
        return s.trim();
    }
