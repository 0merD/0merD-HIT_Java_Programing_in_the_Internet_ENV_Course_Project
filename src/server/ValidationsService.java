...
public class ValidationsService {
    ...
    public static String validateCustomerName(String name) throws IllegalArgumentException {
        ...
        if (name.length() < 3) {
            throw new IllegalArgumentException("Customer name must be at least 3 characters long.");
        }

        return name.trim();
    }

    public static String validateId(String userId) throws IllegalArgumentException {
        ...
        for (User user : UserManager.getInstance().getAllUsers()) {
            if (user.getId().equals(userId.trim())) {
                throw new IllegalArgumentException("ID already exists: " + userId);
            }
        }
    ...
    public static int validateBranchNumber(String branchStr) throws IllegalArgumentException {
        ...
        if (branch <= 0) {
            throw new IllegalArgumentException("Branch number must be positive.");
        }

        // Check if the branch exists in InventoryManager
        if (!InventoryManager.branchExists(branch)) {
            throw new IllegalArgumentException("Branch number " + branch + " does not exist in inventory.");
        }
